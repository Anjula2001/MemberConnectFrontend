...
export default function ProfileChangeRequests() {
  ...
  const getRequestId = (row: RequestData) =>
    row.id ||
    row.Id ||
    row.ID ||
    row.applicationId ||
  ...
  const isSelectableForBoardApproval = (row: RequestData) => {
    ...
    return (
      (requestType === 'Name Changes' || requestType === 'Nomminne Changes') &&
      status !== 'approved' &&
      status !== 'approvedforapproval'
    );
  };
  ...
  const handleSaveBoardMeeting = async () => {
    ...
    try {
      ...
      if (requestType === 'Name Changes') {
        ...
        // Build nomineeChangeRequestIds from selected rows (use request id keys)
        const nomineeChangeRequestIds = normalizedRequestIds
          .map((sel) => {
            const row = results.find((r) => getSelectionKey(r) === sel);
            const id = row ? getRequestId(row) : undefined;
            const n = typeof id === 'string' ? Number(id) : id;
        ...
        if (nomineeChangeRequestIds.length === 0) {
          alert('The selected rows could not be resolved to nominee change request IDs.');
          return;
        }

        payload.nomineeChangeRequestIds = nomineeChangeRequestIds;
      } else {
    ...
    <div className="p-6 bg-[#F9FAFB] min-h-screen">
      ...
      <div className="max-w-7xl mx-auto bg-white p-6 rounded-xl border border-gray-200 shadow-sm mb-8">
        ...
        <div className="mt-6 flex justify-end gap-3">
          {(requestType === 'Name Changes' || requestType === 'Nomminne Changes') && hasSearched && (
            <button
        ...
        <div className="max-w-7xl mx-auto bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <table className="w-full text-left">
            <thead className="bg-[#FDFDFD] border-b border-gray-100 text-gray-500 text-sm">
              <tr>
                {(requestType === 'Name Changes' || requestType === 'Nomminne Changes') && <th className="p-4 w-10"><input type="checkbox" className="h-4 w-4 rounded border-gray-300 text-[#8B3205] focus:ring-[#8B3205]" checked={displayedResults.filter((row) => isSelectableForBoardApproval(row)).length > 0 && displayedResults.filter((row) => isSelectableForBoardApproval(row)).every((row) => selectedRequestIds.includes(getSelectionKey(row)))} onChange={(e) => toggleAllRequests(e.target.checked)} /></th>}
